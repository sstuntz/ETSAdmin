...
Public Class ar_cash_batch
    ...
    Public Sub bat_tot()
        ...
        un_cash.Text = FormatNumber(CDec(bat_Amt.Text) - tot_amt)
        ...
        UnAppChk.Text = FormatNumber(CDec(txtfields(3).Text) - tot_amt)
    ...
    Private Sub CalcUNappChk()
        ...
        UnAppChk.Text = FormatNumber(CDec(Val(txtfields(3).Text)) - Appchks)
    ...
    Private Sub bat_Amt_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles bat_Amt.KeyPress
        ...
        If KeyAscii = 13 Or KeyAscii = 9 Then
            ...
            Call bat_tot()
            bat_Amt.Text = FormatNumber(CDec(bat_Amt.Text))
            txtfields(5).Focus()
            KeyAscii = 0
        End If
    ...
    Private Sub ar_cash_batch_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        refs = ETSSQL.GetRefData("AR", "ar_cash_batch")
        ...
        Select Case entry_type
            ...
            Case "ADD_EDIT"
                ...
                bat_Amt.Text = FormatNumber(RowData.Item(33).ActualData)
                sqlstmnt = "select top (1) entry_num as TheValue from cash_tmp1  where batch_num = '" & selected_bat_num & "' order by entry_num DESC"
                saved_entry_num = CInt(ETSSQL.GetOneSQLValue(sqlstmnt))
        End Select
        ...
        If KeyAscii = 13 Or KeyAscii = 9 Then
            ...
            Select Case Index
                ...
                Case 2 ' check to see if contract
                    ...
                    TextBox12.Text = ETSSQL.GetOneSQLValue("select cont_desc as TheValue from cc_cont where contract_key = '" & selected_contract_key & "'")
                    TextBox4.Focus()
                    Exit Sub
                Case 3 'are amounts
                    If Not IsNumeric(txtfields(Index).Text) Then txtfields(Index).Text = "0"
                    selected_proc_dol_actual = CDec(txtfields(Index).Text)
                    txtfields(Index).Text = FormatNumber(CDec(txtfields(Index).Text))
                    Call CalcUNappChk()
                ...
                Case 5 'invoice
                    ...
                    Else
                        ...
                        If ETSCommon.CheckNumRecords(sqlstmnt) = 0 Then
                            MsgBox("This invoice number does not exist.")
                            GoTo EventExitSub
                    ...
                    TextBox1.Text = FormatNumber(CDec(TextBox1.Text))
...
EventExitSub:
    ...
    Private Sub SaveRP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRP.Click
        ...
        NewCashReceipt.Memo = txtfields(13).Text
        NewCashReceipt.Donor = TextBox2.Text

        If NewCashReceipt.Invoice.Length = 0 Then
            NewCashReceipt.Invoice = "MISC"
        End If

        If NewCashReceipt.CrAcctNu.Length = 0 Then
            MsgBox("Blank CR account number. Receipt not processed.")
            _txtfields_2.Focus()
        End If
    ...
    Private Sub CheckData()
        ...
        If ETSCommon.CheckNumRecords("select * from chacct where acct_num = '" & TextBox10.Text & "'") = 0 Then
            MsgBox("You must enter a VALID cr account.")
            TextBox10.Focus()
            ErrFlag = 1
            Exit Sub
        End If

        If ETSCommon.CheckNumRecords("select * from chacct where acct_num = '" & TextBox11.Text & "'") = 0 Then
            MsgBox("You must enter a VALID dr account.")
            TextBox11.Focus()
            ErrFlag = 1
            Exit Sub
        End If
    ...
    Private Sub PutCashHeader()
        ...
        End If
        TmpCashReceiptHeader.EntryDate = CDate(Today.ToShortDateString)
    ...
    Private Sub FinishReceipt()
        ...
        sqlstmnt = "update cash_tmp1 set batch_desc = '" & txtfields(10).Text & "' , batch_total = '" & bat_Amt.Text & "' where batch_num = '" & txtfields(15).Text & "'"
        ETSSQL.ExecuteSQL(sqlstmnt)
        Call RePaintHeader()
    ...
    Private Sub RePaintHeader()
        ...
        Else
            ...
            txtfields(10).Text = RowData.Item(31).ActualData
            txtfields(15).Text = RowData.Item(30).ActualData
            bat_Amt.Text = FormatNumber(CDec(RowData.Item(33).ActualData))
            sqlstmnt = "select top (1) entry_num as TheValue from cash_tmp1  where batch_num = '" & selected_bat_num & "' order by entry_num DESC"
            saved_entry_num = CInt(ETSSQL.GetOneSQLValue(sqlstmnt))
            txtfields(8).Text = CDate(RowData.Item(6).ActualData).ToShortDateString
            txtfields(3).Text = FormatNumber(CDec(RowData.Item(7).ActualData.ToString))
            ...
            Call calc_tot_inv()
            bat_Amt.Text = FormatNumber(CDec(bat_Amt.Text))
            last_amt.Text = FormatNumber(CDec(last_amt.Text))
            txtfields(3).Text = FormatNumber(CDec(txtfields(3).Text))
        End If
    End Sub
...
EventExitSub:
    ...
    Private Sub TextBox3_keypress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TextBox3.KeyPress
        ...
        If e.KeyChar = Microsoft.VisualBasic.ChrW(Keys.Return) Or e.KeyChar = Microsoft.VisualBasic.ChrW(Keys.Tab) Then
            ...
            End If
            If Strings.Len(TextBox3.Text) = 0 Then
                MsgBox("You must enter a check number.")
            End If
...
EventExitSub:
    ...
    Private Sub textbox10_leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles TextBox10.Leave, TextBox11.Leave
        TextBox6.Text = ETSSQL.GetOneSQLValue("select acctdesc as TheValue from chacct where acct_num = '" & TextBox10.Text & "'")
        TextBox8.Text = ETSSQL.GetOneSQLValue("select acctdesc as TheValue from chacct where acct_num = '" & TextBox11.Text & "'")
