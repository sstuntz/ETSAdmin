...
Public Class ar_cash_edit_sel
    ...
    Private Sub rebuild_grid()  'ByVal sql As String)
        ...
        sqlstmnt = "Select chk_alloc, invoice, cr_acct_nu,  entry_num, memo, donor, chk_num from cash_tmp1 where batch_num = '" & selected_bat_num & "' order by entry_num"
    ...
    Private Sub ar_cash_edit_sel_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        DataGridView1.Columns.Add("Donor", "From:")
        DataGridView1.Columns.Item(2).ReadOnly = False
        DataGridView1.Columns.Item(2).DataPropertyName = "Donor"
        DataGridView1.Columns.Add("Chk_num", "CHK #")
        DataGridView1.Columns.Item(3).DataPropertyName = "chk_num"
        DataGridView1.Columns.Item(3).ReadOnly = False
        DataGridView1.Columns.Add("cr_acct_nu", "ACCT #")
        DataGridView1.Columns.Item(4).DataPropertyName = "cr_Acct_nu"
        DataGridView1.Columns.Item(4).ReadOnly = False
        DataGridView1.Columns.Add("invoice", "Invoice")
        DataGridView1.Columns.Item(5).DataPropertyName = "invoice"
        ...
        DataGridView1.Columns.Item(6).DataPropertyName = "entry_num"
        ctrform(Me)
        selectedcell = False
    ...
    Private Sub Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Save.Click
        ...
        For Each row As DataGridViewRow In DataGridView1.Rows
            'change chk_alloc, net_Check, gross amt where invoice = '" & row.Cells("invoice").Value.ToString & "' and entry_num = '" & row.Cells("entry_num").Value.ToString & "'"  'where batch_num = '" & selected_bat_num & "' and entry_num = '" & selected_line_num & "'"
            sqlstmnt = "update cash_tmp1 set chk_alloc = '" & row.Cells("chk_alloc").Value.ToString & "'  , gross_Amt = '" & row.Cells("chk_alloc").Value.ToString & "' , memo = '" & row.Cells("memo").Value.ToString & "' , donor = '" & row.Cells("donor").Value.ToString & "' , chk_num = '" & row.Cells("chk_num").Value.ToString & "' , cr_acct_nu = '" & row.Cells("cr_acct_nu").Value.ToString & "' where batch_num = '" & selected_bat_num & "'  and entry_num = '" & row.Cells("entry_num").Value.ToString & "'"
            ETSSQL.ExecuteSQL(sqlstmnt)
        Next
