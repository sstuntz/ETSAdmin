...
Public Class ar_det_app
    ...
    Private Sub ar_det_app_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        Select Case function_type
            ...
            Case "VENDOR"
                If Len(SearchTerm) = 0 Then
                    ...
                    sqlstmnt = "select *, (isnull(bal_due,0) - isnull(chk_alloc,0)) as newbal from invoice  left outer join cash_tmp1 on invoice.inv_num = cash_tmp1.inv_num and invoice.line_num = cash_tmp1.line_num where invoice.void = 'N' and invoice.name_key = '" & SearchTerm & "' order by invoice.invoice, invoice.line_num"
                Else
                    'sort by invoice.name_key  and encumbered date
                    sqlstmnt = "select *, (isnull(bal_due,0) - isnull(chk_alloc,0)) as newbal from invoice  left outer join cash_tmp1 on invoice.inv_num = cash_tmp1.inv_num and invoice.line_num = cash_tmp1.line_num where  invoice.void = 'N' and invoice.name_key = '" & SearchTerm & "' and invoice.encum_Date = '" & SearchDate & "' order by invoice.invoice, invoice.line_num"
                End If
            Case "CLIENT"
                'sort by invoice.cc_num
                sqlstmnt = "select *, (isnull(bal_due,0) - isnull(chk_alloc,0)) as newbal from invoice  left outer join cash_tmp1 on invoice.inv_num = cash_tmp1.inv_num and invoice.line_num = cash_tmp1.line_num where invoice.void = 'N' and invoice.cc_num = '" & selected_name_key & "' order by invoice.cc_num"
            Case Else
                ...
                sqlstmnt = sqlstmnt & "where invoice.paid = 'N' and invoice.void = 'N' and invoice.invoice = '" & selected_invoice & "' order by invoice.sort_name"
        ...
        ' old one 
        '     Grid1.AddItem invoice.Recordset.Fields("inv_num") & Chr(9) & invoice.Recordset.Fields("line_num") & Chr(9) & invoice.Recordset.Fields("invoice") 
        '& Chr(9) & bal_Due & Chr(9) & bal_Due & Chr(9) & invoice.Recordset.Fields("trans_code") & Chr(9) & invoice.Recordset.Fields("po_num") 
        '& Chr(9) & invoice.Recordset.Fields("cc_num") & Chr(9) & invoice.Recordset.Fields("cc_name") & Chr(9) & invoice.Recordset.Fields("inv_Desc") 
        '& Chr(9) & invoice.Recordset.Fields("dr_acct_nu") & Chr(9) & invoice.Recordset.Fields("cr_Acct_nu") & Chr(9) 
        '& invoice.Recordset.Fields("alloc_amt") & Chr(9) & invoice.Recordset.Fields("invoice_Date") & Chr(9) & invoice.Recordset.Fields("inv_Amt"), num
