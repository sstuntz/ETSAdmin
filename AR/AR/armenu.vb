...
Public Class armenu
    ...
    Public Sub pap_inv_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles pap_inv.Click
        ...
        For Each ADV In advlist
            ...
            'if chk-amt = total of invoice lisnes pa it all off
            Dim TotInv As Double = CDbl(ETSSQL.GetOneSQLValue("select sum(bal_due) as thevalue from invoice where invoice = '" & ADV.Invoice & "'"))
            If TotInv = ADV.CheckAmt Then
                sqlstmnt = "update invoice set paid_Date = '" & ADV.EncumDate & "' , chk_num = '" & ADV.ChkNum _
                                  & "' , bank_key = '" & ADV.BankKey & "' ,pay_amt = bsl_due, bal_due = 0, paid = 'Y' where invoice = '" _
                                  & ADV.Invoice & "'"
                ETSSQL.ExecuteSQL(sqlstmnt)
            Else
