...
Public Class ar_bat_lookup
    ...
    Dim ChkAccts As Integer
    Dim ExcelApp As Microsoft.Office.Interop.Excel.Application
    Dim ExcelBook As Microsoft.Office.Interop.Excel.Workbook

    Private Sub rebuild_grid()  'ByVal sql As String)
        Dim sql As String = " select distinct t.batch_num, batch_Date, batch_Desc, batch_total, unapp_total from cash_tmp1 T inner join ( select  batch_num,  (batch_total - sum(chk_alloc)) as unapp_total  from cash_tmp1  group by batch_num, batch_total) TT on t.batch_num = tt.batch_num order by t.batch_num desc"
    ...
    Private Sub ed_det_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles ed_det.Click
        ...
        MsgBox("Are you sure you want to delete batch # " & batch_num, CType(MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, MsgBoxStyle))
    ...
    Private Sub extra_com_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles extra_com.Click
        ...
        For Each row As DataGridViewRow In DataGridView1.Rows
            If row.Selected = True Then
                ...
                'select COUNT(*) as thevalue from cash_tmp1  left outer join chacct on chacct.jriacct = cash_tmp1.dr_acct_nu where chacct.jriacct is null and batch_num = '9989'

                ChkAccts = CInt(ETSSQL.GetOneSQLValue("select COUNT(*) as thevalue from cash_tmp1  left outer join chacct on chacct.jriacct = cash_tmp1.dr_acct_nu where chacct.jriacct is null and batch_num = '" & selected_bat_num & "'"))
                If ChkAccts <> 0 Then
                    MsgBox("BATCH NOT POSTED Invalid DR account number on entry number = " & ChkAccts)
                    Exit Sub
                End If
                ChkAccts = CInt(ETSSQL.GetOneSQLValue("select COUNT(*) as thevalue from cash_tmp1  left outer join chacct on chacct.jriacct = cash_tmp1.cr_acct_nu where chacct.jriacct is null and batch_num = '" & selected_bat_num & "'"))
                If ChkAccts <> 0 Then
                    MsgBox("BATCH NOT POSTED Invalid CR account number on entry number = " & ChkAccts)
                    Exit Sub
                End If
                ChkAccts = CInt(ETSSQL.GetOneSQLValue("select COUNT(*) as thevalue from cash_tmp1  left outer join chacct on chacct.jriacct = cash_tmp1.disc_acct where chacct.jriacct is null and batch_num = '" & selected_bat_num & "'"))
                'If ChkAccts = 0 Then
                ...
                For Each Cashitem In CashRec
                    If Len(Cashitem.InvNum) <> 0 Then
                        ...
                        sqlstmnt = sqlstmnt & ", pay_amt = '" & Cashitem.ChkAlloc & "'"
    ...
    Private Sub FixEntryNumbers()
        ...
        'do it by inv inv line numbre order in batch
