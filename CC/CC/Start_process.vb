...
Friend Class start_process
    ...
    Private Sub beg_chk_num_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles beg_chk_num.KeyPress
        ...
        If KeyAscii = 13 Or KeyAscii = 9 Then
            ...
            valid_check = "N"
            Call prchknumverify(CInt(beg_chk_num.Text))
...
EventExitSub:
    ...
    Private Sub pr_Start_date1_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles pr_Start_date1.Enter
        ...
        Dim last_ckdate As Date
        sqlstmnt = "select * from cccktmp"
        Using db As Database = New Database(ConfigurationManager.AppSettings("connectionString"))
            Dim rs As SqlDataReader = db.ExecuteQuery(sqlstmnt)
            While rs.Read()
                YNCount = YNCount + 1
                ' last_ckdate = CDate(rs("chk_Date"))
            End While
            rs.Close()
        End Using

        If YNCount = 0 Then
            MsgBox("There is no data in check temp file. A payroll can not be run.")
            Exit Sub
        End If

        Response = MsgBox("Do you wish to process a payroll without running another edit?", CType(MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, MsgBoxStyle))
        If Response = vbNo Then Exit Sub
        num_cks.Text = YNCount.ToString
        num_emp.Text = YNCount.ToString
        pr_Start_date1.Text = last_ckdate.ToShortDateString
...
EventExitSub:
    ...
    Private Sub Process_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Process.Click
        ...
        If single_pr.Checked Then 'single check payroll
            sqlstmnt = "select distinct name_key, entry_num from cctime where paid = 'N' and void = 'N' and pay_num = '" & Pr_num1.Text & "'  and name_key = '"
            sqlstmnt = sqlstmnt & selected_name_key & "' order by name_key,entry_num"
        Else
            sqlstmnt = "select distinct name_key, entry_num from cctime where paid = 'N' and void = 'N' and pay_num = '" & Pr_num1.Text & "' order by name_key,entry_num"
        End If
        ...
        Using db As Database = New Database(ConfigurationManager.AppSettings("connectionString"))
            ...
            sqlstmnt = " INSERT INTO cccktmp (name_key, dir_hrs, dir_dol) SELECT  name_key, SUM(hours) AS shours, SUM(pay_dol) AS sdol FROM cctime WHERE (paid <> 'Y') AND (void = 'N') AND (pay_num = '" & Pr_num1.Text & "' ) GROUP BY name_key, entry_num"
            sql = sqlstmnt
            db.ExecuteUpdate(sqlstmnt)

            'fills from nam cc
            sqlstmnt = "Update cccktmp SET fwt_exmts = nam_cc.fwt_exmts, swt_exmts = nam_cc.swt_exmts, dept_num = nam_cc.dept_num, fund_code = nam_cc.fund_code, sort_name = nam_cc.sort_name, screen_nam = nam_cc.screen_nam, state1_code = nam_cc.state1_tax, desc1 = nam_cc.desc1, desc2 = nam_cc.desc2, desc3 = nam_cc.desc3,chk_dirdep = nam_cc.ddep_net "
            ...
            sqlstmnt = " Update cccktmp set chk_Date = '" & pr_Start_date & "' "
            db.ExecuteUpdate(sqlstmnt)
            ...
            sqlstmnt = " update ETSSQL.dbo.cccktmp set net_pay = full_gross - fed_tax - fica_tax - med_tax - state1_tax - state2_tax - dduct1_dol - dduct2_dol - dduct3_dol - dduct4_dol - add_fwt - add_swt - add_fica - add_med "
            db.ExecuteUpdate(sqlstmnt)
    ...
    '        num = 0
        sqlstmnt = "update cccktmp set chk_Date = '" & pr_Start_date1.Text & "', chk_num = '" & CInt(beg_chk_num.Text) + 1 & "'"
        Using db As Database = New Database(ConfigurationManager.AppSettings("connectionString"))
    Dim sql As String = sqlstmnt
            db.ExecuteQuery(sql)
        End Using

    '        chks_prted.Text = CStr(num) 'fills in the number of checks printed
        ...
        If Response = 6 Then
            ...
            Else
                ...
                If CDbl(beg_chk_num.Text) - last_Chk_num > 1 Then
                    ...
                    For num = 2 To (CInt(beg_chk_num.Text) - last_Chk_num - 1)
                ...
                End If
            End If
            'put check numbers into cccktmp
            sqlstmnt = " WITH CTE AS (SELECT chk_num, chk_date, chk_dirdep, pay_num, name_key, screen_nam, sort_name, entry_num, row_number() OVER (ORDER BY name_key, entry_num) AS row"
            sqlstmnt = sqlstmnt & " FROM cccktmp)    Update cte SET  chk_num = row +  '" & last_Chk_num & "'"
            Using db As Database = New Database(ConfigurationManager.AppSettings("connectionString"))
                Dim sql As String = sqlstmnt
                db.ExecuteQuery(sql)
            End Using
            ...
            sqlstmnt = " UPDATE cctime set  pay_dol = derivedtbl_1.minimum, date = '" & end_Date.Text & "', job_num = '88887', pay_class = 'SP', hours = '0', pcs_prod = '0', pay_dol = 'x', cl_pct = '0', line_num = '99'"
            ...
            sqlstmnt = " UPDATE cctime set  pay_dol = derivedtbl_1.makeup, date = '" & end_Date.Text & "', job_num = '88887', pay_class = 'SP', hours = '0', pcs_prod = '0', pay_dol = 'x', cl_pct = '0', line_num = '99'"
            ...
            Call update_job(1, CInt(Pr_num1.Text))  ' for the whole year
            ...
            If Response = MsgBoxResult.Yes Then
                ...
                '                    cctime.Recordset.Edit()
                '                    cctime.Recordset.Fields("pay_num").Value = new_pr
                '                    cctime.Recordset.Update()
                '                    cctime.Recordset.MoveNext()
                '                Loop
                '            End If

                '            'here we zero the cktmp file
