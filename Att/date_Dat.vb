...
Friend Class ed_dates
    ...
    Private Sub cmdUpdate_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdUpdate.Click
        ...
        Next
        For num As Short = 0 To 41 'changed from start_position to cover beginning of month 10/14/00
            If sdr(num).Text <> "" Then
                sqlstmnt = "insert into attend_tmp "
        ...
        For num As Short = 0 To 41
            EdAttendData.Item(num).AttCode = sdr(num).Text
            EdAttendData.Item(num).AttDate = date_Array(num)
            If selected_bill_type = "125" Then
                EdAttendData.Item(num).AttUnit = Val(sdr(num).Text)
            End If
            If selected_bill_type = "120" Then
                ...
                Else
                    If InStr(billable_codes, sdr(num).Text) = 0 Then 'And Len(sdr(num)) = 0 Then 'sdr(num) <> "X" And sdr(num) <> "A" And sdr(num) <> "B" Then
                        EdAttendData.Item(num).AttUnit = 0
                    Else
                        EdAttendData.Item(num).AttUnit = 1 'added 8/12/99 for counting problems
                    End If
                End If
                EdAttendData.Item(num).ClientPct = (Val(client_pct.Text) + 0) / 100
            End If
